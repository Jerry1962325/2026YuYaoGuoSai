...
特性：
  ...
  - 指针实例按"铆钉离表盘圆心最近"筛选，避免误检画面其他细长物
  - 相对角度滑动平均，抑制区域边界抖动
...
def recognize_letter_box(frame, gauge_box, debug=False, debug_dir=None, debug_idx=0):
    ...
    candidates = [
        # 表盘框上方区域：放宽到 1.5 倍框高、左右各 0.3 倍框宽。
        # 字母经常离表盘较远或贴近画面顶边，原来 0.8 倍框高经常包不住字母
        (max(0, int(x1 - box_w * 0.3)),
         max(0, int(y1 - box_h * 1.5)),
         min(w, int(x2 + box_w * 0.3)),
         y1,
         'above'),
...
class GaugeYOLORecognizer:
    def __init__(self, models_dir, use_engine=True, device=0, imgsz=640,
        ...
        self.device = device
        self.imgsz = imgsz
        # 区域边界：相对角 <= thr_high 为偏高，thr_high~thr_low 为居中，其余偏低
        self.thr_high = thr_high
        self.thr_low = thr_low
        # 最近几帧的相对角度，用于滑动平均抑制区域边界抖动
        self._rel_history = deque(maxlen=5)
    ...
    @staticmethod
    def classify(rel_angle, thr_high=41.0, thr_low=145.0):
        """
        根据相对角度划分区域：偏高/居中/偏低。

        阈值由第一版数据集标注算出（量程 0~1MPa 共 263.7°，红框中心 ≈355°）：
          红绿边界 0.7MPa -> rel ≈ +39°，指针越过红区中心后 rel 变负，
          1.0MPa -> rel ≈ -40°，所以偏高必须是双侧对称区间 |rel| <= thr_high，
          thr_high=41 可覆盖整个红区（含满量程 1.0MPa）；
          黄绿边界 0.3MPa -> rel ≈ +144°，故 thr_low=145。
        """
        rel = rel_angle % 360
        if rel > 180:
            rel -= 360
    ...
    def process(self, frame):
        ...
        # 可能检出多个"指针"实例（画面里其他细长物、纸角等），
        # 取铆钉离表盘圆心最近的一个；都离得太远则认为本帧未检出
        best = None
        best_dist = None
        for xy in kpts.xy:
            p = xy.cpu().numpy()
            if len(p) < 2:
                continue
            dist = math.hypot(p[0][0] - cx, p[0][1] - cy)
            if best is None or dist < best_dist:
                best = p
                best_dist = dist

        if best is None or best_dist > r * 1.5:
            return None, None, frame.copy(), None, None

        rivet = (int(best[0][0]), int(best[0][1]))
        tip = (int(best[1][0]), int(best[1][1]))
        ptr_angle = self.angle(rivet[0], rivet[1], tip[0], tip[1])

        # 3. 计算区域：对相对角做圆均值滑动平均，抑制在区域边界附近来回跳
        rel_raw = (ptr_angle - up_angle) % 360
        self._rel_history.append(rel_raw)
        sin_sum = sum(math.sin(math.radians(a)) for a in self._rel_history)
        cos_sum = sum(math.cos(math.radians(a)) for a in self._rel_history)
        rel_smooth = math.degrees(math.atan2(sin_sum, cos_sum)) % 360
        status, color = self.classify(rel_smooth, self.thr_high, self.thr_low)
        ...
        # 注意：angle() 用的是数学坐标系（y 向上），画回 OpenCV 图像时 sin 前要取负号，
        # 否则箭头会变成真实方向的上下镜像
        ...
        gauge_info = {'cx': cx, 'cy': cy, 'r': r, 'rel': rel_smooth}
...
class AsyncYOLOGaugeProcessor:
    ...
    def _print_state(self, state):
        ...
        rel = state.get('rel')
        rel_str = f",rel={rel:.0f}°" if rel is not None else ""
        print(f"{letter},{region},{state_cn}{rel_str}")
...
def main():
    ...
    parser.add_argument('--thr-high', type=float, default=41.0,
                        help='偏高/居中边界（相对角度，双侧对称），按红绿边界 0.7MPa 校准，默认 41')
    parser.add_argument('--thr-low', type=float, default=145.0,
                        help='居中/偏低边界（相对角度），按黄绿边界 0.3MPa 校准，默认 145')
