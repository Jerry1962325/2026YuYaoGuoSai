...
Broadcast mode:
  Loads a recorded path, sends reset_origin, then publishes /move commands
  sequentially to drive the robot through the waypoints.
...
class WaypointTool(Node):
    def __init__(self, mode, filepath):
        ...
        if mode == "record":
            ...
            self._load()
            self.get_logger().info(
                f"Broadcast mode. Loaded {len(self._waypoints)} waypoints, "
    ...
    def _load(self):
        ...
        self._moves = data.get("moves") or compute_moves(self._waypoints)
    ...
    def _is_at(self, waypoint):
        with self._lock:
            if self._current is None:
                return False
            dx = self._current["x"] - waypoint["x"]
            dy = self._current["y"] - waypoint["y"]
            dyaw = normalize_angle(self._current["yaw"] - waypoint["yaw"])
            dist = math.hypot(dx, dy)
            return dist < ARRIVE_DIST and abs(dyaw) < ARRIVE_YAW
    ...
    def _tick_broadcast(self):
        ...
        if self._broadcast_state == "init":
            ...
            target = self._waypoints[self._broadcast_index + 1]
            if self._is_at(target):
                self.get_logger().info(
                    f"Arrived at waypoint {self._broadcast_index + 1}"
                )
                self._broadcast_index += 1
                self._broadcast_state = "moving"
            elif (now - self._wait_start).nanoseconds / 1e9 >= MOVE_TIMEOUT:
                self.get_logger().warn(
                    f"Timeout waiting for waypoint {self._broadcast_index + 1}, skipping"
                )
                self._broadcast_index += 1
                self._broadcast_state = "moving"
...
def main(argv=None):
    ...
    if len(argv) >= 1:
        mode = argv[0].lower()
        filepath = argv[1] if len(argv) > 1 else DEFAULT_PATH
    else:
        mode = _select_mode()
